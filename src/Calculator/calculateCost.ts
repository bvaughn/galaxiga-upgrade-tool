...
export function calculateCost(
  ...
  // let todo = `level: ${stats.level} (of ${MAX_LEVEL}), upgrade: ${stats.upgrade} (of ${MAX_UPGRADE})`;
  ...
  if (stats.level === MAX_LEVEL && stats.upgrade !== MAX_UPGRADE) {
    const levelCosts = TIER_1_SHIP_AND_STONE[TIER_1_SHIP_AND_STONE.length - 1];

    for (
      let goldCostIndex = stats.upgrade;
      goldCostIndex < MAX_UPGRADE;
      goldCostIndex++
    ) {
      // todo += `\n    upgrade: ${goldCostIndex + 1}`;
